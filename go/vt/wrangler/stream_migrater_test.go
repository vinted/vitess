...
func TestStreamMigrateMainflow(t *testing.T) {
	...
	stopStreams := func() {
		...
		tme.dbSourceClients[0].addQuery("select id, workflow, source, pos, workflow_type from _vt.vreplication where db_name='vt_ks' and workflow != 'test_reverse' and state = 'Stopped' and message != 'FROZEN'", &sqltypes.Result{}, nil)
		tme.dbSourceClients[1].addQuery("select id, workflow, source, pos, workflow_type from _vt.vreplication where db_name='vt_ks' and workflow != 'test_reverse' and state = 'Stopped' and message != 'FROZEN'", &sqltypes.Result{}, nil)
		...
		for _, sourceTargetShard := range tme.sourceShards {
			...
			for j, sourceShard := range tme.sourceShards {
				...
				rows = append(rows, fmt.Sprintf("%d|t1t2|%v|MariaDB/5-456-888|0", j+1, bls))
			}
		...
		for i, dbclient := range tme.dbSourceClients {
			...
			dbclient.addQuery("select id, workflow, source, pos, workflow_type from _vt.vreplication where db_name='vt_ks' and workflow != 'test_reverse'", sqltypes.MakeTestResult(sqltypes.MakeTestFields(
				"id|workflow|source|pos|workflow_type",
				"int64|varbinary|varchar|varbinary|int64"),
				sourceRows[i]...),
				nil)
			...
			dbclient.addQuery("select id, workflow, source, pos, workflow_type from _vt.vreplication where db_name='vt_ks' and workflow != 'test_reverse' and id in (1, 2)", sqltypes.MakeTestResult(sqltypes.MakeTestFields(
				"id|workflow|source|pos|workflow_type",
				"int64|varbinary|varchar|varbinary|int64"),
				sourceRows[i]...),
				nil)
	...
	stopStreams := func() {
		...
		for _, sourceTargetShard := range tme.sourceShards {
			...
			for j, sourceShard := range tme.sourceShards {
				...
				rows = append(rows, fmt.Sprintf("%d|t3|%v|MariaDB/5-456-888|0", j+3, bls))
			}
			sourceRows = append(sourceRows, rows)
		}
		...
		for i, dbclient := range tme.dbSourceClients {
			...
			// sm.stopStreams->sm.stopSourceStreams->sm.readTabletStreams('id in...')
			dbclient.addQuery("select id, workflow, source, pos, workflow_type from _vt.vreplication where db_name='vt_ks' and workflow != 'test_reverse' and id in (1, 2, 3, 4)", sqltypes.MakeTestResult(sqltypes.MakeTestFields(
				"id|workflow|source|pos|workflow_type",
				"int64|varbinary|varchar|varbinary|int64"),
				sourceRows[i]...),
				nil)
	...
	stopStreams := func() {
		...
		for _, sourceTargetShard := range tme.sourceShards {
			...
			for j, sourceShard := range tme.sourceShards {
				...
				rows = append(rows, fmt.Sprintf("%d|t1|%v|MariaDB/5-456-888|0", j+1, bls))
			}
			sourceRows = append(sourceRows, rows)
		}
		...
		for i, dbclient := range tme.dbSourceClients {
			...
			// sm.stopStreams->sm.stopSourceStreams->sm.readTabletStreams('id in...')
			dbclient.addQuery("select id, workflow, source, pos, workflow_type from _vt.vreplication where db_name='vt_ks' and workflow != 'test_reverse' and id in (1)", sqltypes.MakeTestResult(sqltypes.MakeTestFields(
				"id|workflow|source|pos|workflow_type",
				"int64|varbinary|varchar|varbinary|int64"),
				sourceRows[i]...),
				nil)
		...
		for i, sourceTargetShard := range tme.sourceShards {
			...
			for j, sourceShard := range tme.sourceShards {
				...
				case 0:
					switch j {
					case 0:
						rows = append(rows, fmt.Sprintf("%d|t1|%v|MariaDB/5-456-887|0", j+1, bls))
					case 1:
						rows = append(rows, fmt.Sprintf("%d|t1|%v|MariaDB/5-456-888|0", j+1, bls))
					}
				case 1:
...
func TestStreamMigrateStoppedStreams(t *testing.T) {
	...
	stopStreams := func() {
		...
		for i, dbclient := range tme.dbSourceClients {
			// sm.stopStreams->sm.readSourceStreams->readTabletStreams('') and VReplicationExec(_vt.copy_state)
			dbclient.addQuery("select id, workflow, source, pos, workflow_type from _vt.vreplication where db_name='vt_ks' and workflow != 'test_reverse' and state = 'Stopped' and message != 'FROZEN'", sqltypes.MakeTestResult(sqltypes.MakeTestFields(
				"id|workflow|source|pos|workflow_type",
				"int64|varbinary|varchar|varbinary|int64"),
				sourceRows[i]...),
				nil)
	...
	stopStreams := func() {
		...
		for _, sourceTargetShard := range tme.sourceShards {
			...
			for j, sourceShard := range tme.sourceShards {
				...
				rows = append(rows, fmt.Sprintf("%d||%v|MariaDB/5-456-888|0", j+1, bls))
			}
			sourceRows = append(sourceRows, rows)
		}
	...
	stopStreams := func() {
		...
		for _, sourceTargetShard := range tme.sourceShards {
			...
			for j, sourceShard := range tme.sourceShards {
				...
				rows = append(rows, fmt.Sprintf("%d|test|%v|MariaDB/5-456-888|0", j+1, bls))
			}
			sourceRows = append(sourceRows, rows)
		}
