...
func (tme *testMigraterEnv) createDBClients(ctx context.Context, t *testing.T) {
	...
	for _, master := range tme.sourceMasters {
		...
		master.TM.VREngine = vreplication.NewTestEngine(tme.ts, master.Tablet.GetAlias().GetCell(), master.FakeMysqlDaemon, dbClientFactory, dbClientFactory, dbclient.DBName(), nil)
