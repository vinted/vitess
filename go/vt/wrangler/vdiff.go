...
func (df *vdiff) stopTargets(ctx context.Context) error {
	...
	err := df.forAll(df.targets, func(shard string, target *shardStreamer) error {
		query := fmt.Sprintf("update _vt.vreplication set state='Stopped', message='for vdiff' where db_name=%s and workflow=%s", encodeString(target.master.DbName()), encodeString(df.ts.WorkflowName()))
		_, err := df.ts.TabletManagerClient().VReplicationExec(ctx, target.master.Tablet, query)
		if err != nil {
			return err
		}
		query = fmt.Sprintf("select source, pos from _vt.vreplication where db_name=%s and workflow=%s", encodeString(target.master.DbName()), encodeString(df.ts.WorkflowName()))
		p3qr, err := df.ts.TabletManagerClient().VReplicationExec(ctx, target.master.Tablet, query)
		if err != nil {
			return err
		}
...
func (df *vdiff) syncTargets(ctx context.Context, filteredReplicationWaitTime time.Duration) error {
	...
	err = df.forAll(df.targets, func(shard string, target *shardStreamer) error {
		pos, err := df.ts.TabletManagerClient().MasterPosition(ctx, target.master.Tablet)
		if err != nil {
			return err
		}
