...
func (vte *VTExplain) initVtgateExecutor(vSchemaStr, ksShardMapStr string, opts *Options) error {
	...
	vte.healthCheck = discovery.NewFakeHealthCheck()
	...
	vte.vtgateExecutor = vtgate.NewExecutor(context.Background(), vte.explainTopo, vtexplainCell, resolver, opts.Normalize, false /*do not warn for sharded only*/, streamSize, cache.DefaultConfig, schemaTracker)
...
func (vte *VTExplain) buildTopology(opts *Options, vschemaStr string, ksShardMapStr string, numShardsPerKeyspace int) error {
	...
	for ks, vschema := range vte.explainTopo.Keyspaces {
		...
		for _, shard := range shards {
			...
			tablet := vte.healthCheck.AddFakeTablet(vtexplainCell, hostname, 1, ks, shard.Name, topodatapb.TabletType_MASTER, true, 1, nil, func(t *topodatapb.Tablet) queryservice.QueryService {
...
func (vte *VTExplain) sortShardSession() {
	if len(vte.vtgateSession.ShardSessions) > 1 {
