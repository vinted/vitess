...
func TestPlayerCopyCharPK(t *testing.T) {
	...
	*copyPhaseDuration = 500 * time.Millisecond
	...
	query := binlogplayer.CreateVReplicationState("test", bls, "", binlogplayer.VReplicationInit, playerEngine.dbName, 0)
...
func TestPlayerCopyTableContinuation(t *testing.T) {
	...
	query := binlogplayer.CreateVReplicationState("test", bls, "", binlogplayer.BlpStopped, playerEngine.dbName, 0)
	qr, err := playerEngine.Exec(query)
	if err != nil {
		t.Fatal(err)
	}
