...
func (tp *TabletPicker) GetMatchingTablets(ctx context.Context) []*topo.TabletInfo {
	// Special handling for MASTER tablet type: since there is only
	// one madter per shard, we ignore cell and find the master.
	...
	if len(tp.tabletTypes) == 1 && tp.tabletTypes[0] == topodatapb.TabletType_MASTER {
		...
		if _, ignore := tp.ignoreTablets[si.MasterAlias.String()]; !ignore {
