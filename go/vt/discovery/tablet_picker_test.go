...
func TestPickPrimary(t *testing.T) {
	...
	tp, err := NewTabletPicker(context.Background(), te.topoServ, []string{"otherCell"}, "cell", te.keyspace, te.shard, "master", TabletPickerOptions{})
	require.NoError(t, err)
	...
	tcases := []testCase{
		{
			...
			tablets: []tablet{
				{100, topodatapb.TabletType_REPLICA, "cell"},
				{101, topodatapb.TabletType_MASTER, "cell"},
			},
			envCells:      []string{"cell"},
			inCells:       []string{"cell"},
			localCell:     "cell",
