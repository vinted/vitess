...
# build the vitess vtexplain binary statically
build_vtexplain:
ifndef NOBANNER
	echo $$(date): Building source tree
endif
	bash ./build.env
	CGO_ENABLED=d0 go install $(EXTRA_BUILD_FLAGS) $(VT_GO_PARALLEL) -ldflags "$(shell tools/build_version_flags.sh)" ./go/cmd/vtexplain/...
